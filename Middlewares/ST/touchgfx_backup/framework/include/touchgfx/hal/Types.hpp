...
public:
    ...
    void operator&=(const Rect& other)
    {
        if (intersect(other))
        {
            ...
            width = USBMIN(right(), other.right()) - newX;
            height = USBMIN(bottom(), other.bottom()) - newY;
            x = newX;
            y = newY;
        }
        else
    ...
    void expandToFit(const Rect& other)
    {
        if (!other.isEmpty())
        {
            ...
            {
                int16_t newX = USBMIN(x, other.x);
                int16_t newY = USBMIN(y, other.y);
