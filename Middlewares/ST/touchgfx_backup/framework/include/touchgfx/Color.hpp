...
public:
    ...
    {
        const uint8_t rgbMin = USBMIN(USBMIN(red, green), blue);
    ...
    {
        value = luminance + LCD::div255(saturation * USBMIN(luminance, 255 - luminance));
    ...
    static void getHSLFromHSV(uint8_t hue, uint8_t& saturation, uint8_t value, uint8_t& luminance)
    {
        ...
        saturation = luminance == 0 || luminance == 255 ? 0 : (((value - luminance) * 255) / USBMIN(luminance, 255 - luminance));
    }
